...
if [ $# -eq 4 ]; then
    if [ $4 = "F" ]; then
       docker run --name $1 -i -t -p $2:8080 -v $3:/usr/local/tomcat/webapps/birt/reports cdorde/mybirt
    else
       if [ $4 = "D" ]; then
          docker run --name $1 -d --restart=always -i -t -p $2:8080 -v $3:/usr/local/tomcat/webapps/birt/reports cdorde/mybirt
       else
